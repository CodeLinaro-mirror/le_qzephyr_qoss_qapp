...
#if defined(QC_OS_FREERTOS) && defined(QCC730_SPI_ENABLE)
#ifdef SHELL_FEATURE
#include "shell.h"
#endif
...
/**
 * @brief Test qcspi write/read performance
 */
static int cmd_qcspi_test_transfer(int argc, char **argv)
{
    ...
    if (elapsed_time > 0) {
        uint64_t throughput_bps = (total_bytes * 8 * 1000) / elapsed_time;
...
/**
 * @brief Send AT command via ring service (auto-append \r\n)
 */
int at_send_cmd(int argc, char **argv)
{
    if (argc < 2) {
        printf("Usage: at_send <string>\r\n");
        printf("Example: at_send AT\r\n");
        printf("Example: at_send AT+TEST\r\n");
        printf("Note: \\r\\n will be automatically appended\r\n");
        return -QC_OSAL_EINVAL;
    }

    uint8_t ring_id = RING_0;
    size_t offset = 0;

    for (size_t i = 1; i < argc && offset < 1500 - 3; i++) {
        size_t arg_len = strlen(argv[i]);
        if (offset + arg_len + 3 > 1500) {
            break;
        }
    ...
    send_buffer[offset++] = '\r';
    send_buffer[offset++] = '\n';
    send_buffer[offset] = '\0';

    printf("Sending AT command (%u bytes): \"%.*s\"\r\n", (unsigned int)offset, (int)(offset - 2), send_buffer);

    int ret = ring_send(ring_id, (const uint8_t *)send_buffer, offset, 1000);
    if (ret < 0) {
        printf("ERROR: AT command send failed: %d\r\n", ret);
        return ret;
    }
...
/* Burst test implementation - keeping existing code */
enum burst_test_mode { BURST_MODE_COUNT, BURST_MODE_TIME, BURST_MODE_DATA };
...
int ring_send_burst(int argc, char **argv)
{
    ...
    struct qc_osal_thread_config config = {
        .name = "burst_test", .stack_size = 4096, .priority = 5, .entry = burst_test_thread, .arg = &burst_params};
...
void spi_demo_init(void)
{
    ...
    printf("SPI demo initialized successfully\r\n");
}
...
void spi_demo_register_commands(void)
{
    ...
    cmd_shell_add("at_send", (void *)at_send_cmd, "Send AT command (auto-append \\r\\n)");
...
#endif /* QC_OS_FREERTOS && QCC730_SPI_ENABLE */
