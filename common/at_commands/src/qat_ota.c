...
/* ----------------------------------------------------------------------- *
 * AT+OTAFWD — Display or delete Firmware Descriptor (FWD) table entries
 * ----------------------------------------------------------------------- */

#define OTAFWD_FWD_MAGIC 0x54445746U  /* FW_UPGRADE_MAGIC_V1 */
#define OTAFWD_FWD_COUNT 3
#define OTAFWD_LINE_BUF  512

static void otafwd_format_partition(fu_part_hdl_t hdl, char *buf, int *off, int buf_size)
{
	fu_partition_client_t *p = (fu_partition_client_t *)hdl;

	*off += snprintf(buf + *off, buf_size - *off, ",0x%X,0x%X,0x%X,0x%X",
			 (unsigned)p->img_id, (unsigned)p->img_version,
			 (unsigned)p->img_start, (unsigned)p->img_size);
}

static cat_return_state cmd_ota_otafwd_run(const struct cat_command *cmd)
{
	ARG_UNUSED(cmd);
	return QAT_Response_Str(QAT_RC_OK,
				"+OTAFWD: display FWD info\r\n"
				"+OTAFWD?: display FWD\r\n"
				"+OTAFWD=del,<fwd_num>: delete FWD");
}

static cat_return_state cmd_ota_otafwd_read(const struct cat_command *cmd, uint8_t *data,
					    size_t *data_size, const size_t max_data_size)
{
	...
	uint32_t boot_type  = 0;
	uint32_t valid_fwd  = 0;
	uint8_t  active_idx;
	char     buf[OTAFWD_LINE_BUF];
	int      off;
	int      i;
	...
	active_idx = fw_upgrade_get_active_fwd(&boot_type, &valid_fwd);
	snprintf(buf, sizeof(buf), "+OTAFWD:%s,%d,%d\r\n",
		 (boot_type & FW_UPGRADE_FWD_BOOT_TYPE_TRIAL)  ? "Trial"   :
		 (boot_type & FW_UPGRADE_FWD_BOOT_TYPE_CURRENT) ? "Current" : "Golden",
		 (int)active_idx, (int)valid_fwd);
	QAT_Output((uint32_t)strlen(buf), buf);

	for (i = 0; i < OTAFWD_FWD_COUNT; i++) {
		uint32_t magic   = 0;
		uint32_t rank    = 0;
		uint32_t version = 0;
		uint8_t  status  = 0;
		...
		fw_upgrade_get_fwd_info((uint8_t)i, FW_UPGRADE_FWD_MAGIC_E,       (uint8_t *)&magic);
		fw_upgrade_get_fwd_info((uint8_t)i, FW_UPGRADE_FWD_RANK_E,        (uint8_t *)&rank);
		fw_upgrade_get_fwd_info((uint8_t)i, FW_UPGRADE_FWD_VERSION_E,     (uint8_t *)&version);
		fw_upgrade_get_fwd_info((uint8_t)i, FW_UPGRADE_FWD_STATUS_E,      &status);
		fw_upgrade_get_fwd_info((uint8_t)i, FW_UPGRADE_FWD_TOTAL_IMAGE_E, &total);

		memset(buf, 0, sizeof(buf));
		off = snprintf(buf, sizeof(buf), "+OTAFWD:%d,0x%X,0x%X,0x%X,0x%X,0x%X",
			       i, (unsigned)magic, (unsigned)rank, (unsigned)version,
			       (unsigned)status, (unsigned)total);

		if (magic == OTAFWD_FWD_MAGIC) {
			fu_part_hdl_t hdl      = NULL;
			fu_part_hdl_t hdl_next = NULL;

			if (fw_upgrade_first_partition((uint8_t)i, &hdl) == FW_UPGRADE_OK_E) {
				otafwd_format_partition(hdl, buf, &off, sizeof(buf));
		...
		QAT_Output((uint32_t)off, buf);
	}

	return CAT_RETURN_STATE_OK;
}

static cat_return_state cmd_ota_otafwd_write(const struct cat_command *cmd, const uint8_t *data,
					     const size_t data_size, const size_t args_num)
{
	ARG_UNUSED(cmd);
	ARG_UNUSED(args_num);
	...
	if (token_count != 2 || strncasecmp(tokens[0], "del", 3) != 0) {
		QAT_Output(sizeof("+OTAFWD:usage: AT+OTAFWD=del,<fwd_num>\r\n") - 1,
			   "+OTAFWD:usage: AT+OTAFWD=del,<fwd_num>\r\n");
		return CAT_RETURN_STATE_ERROR;
	}

	{
		char *end = NULL;
		long  idx = strtol(tokens[1], &end, 10);

		if (!end || *end != '\0' || idx < 0 || idx >= OTAFWD_FWD_COUNT) {
			QAT_Output(sizeof("+OTAFWD:invalid fwd number\r\n") - 1,
				   "+OTAFWD:invalid fwd number\r\n");
			return CAT_RETURN_STATE_ERROR;
		}
		...
		if (fw_upgrade_erase_fwd((uint8_t)idx) != FW_UPGRADE_OK_E) {
			QAT_Output(sizeof("+OTAFWD:Erase Error\r\n") - 1, "+OTAFWD:Erase Error\r\n");
			return CAT_RETURN_STATE_ERROR;
		}

		{
			char resp[40];
			int  n = snprintf(resp, sizeof(resp), "+OTAFWD:Delete FWD%ld\r\n", idx);

			QAT_Output((uint32_t)n, resp);
		}
	}
...
static struct cat_command qat_ota_cmds[] = {
	{
		.name        = "+OTAFWD",
		.description = "Display or manage Firmware Descriptor (FWD) table",
		.run         = cmd_ota_otafwd_run,
		.read        = cmd_ota_otafwd_read,
		.write       = cmd_ota_otafwd_write,
	},
	{
		.name        = "+OTAFWUP",
